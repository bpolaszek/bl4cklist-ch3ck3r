...
class FileHashStorage implements HashStorageInterface
{
    /**
     * @var string
     */
    ...
    {
        return sprintf('%s/%s_%s_%s', $this->storageDirectory, ...func_get_args());
    }
    ...
    private function getRawHashes(string $threatType, string $threatEntryType, string $platformType): array
    {
        ...
        return json_decode(file_get_contents($this->getFilename($threatType, $threatEntryType, $platformType)), true);
    }
    ...
    public function storeHashes(string $threatType, string $threatEntryType, string $platformType, array $additions, array $removals): void
    {
        ...
        file_put_contents($this->tmpfile['source'], json_encode($rawHashes));
    }
